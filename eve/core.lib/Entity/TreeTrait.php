...
trait Entity_TreeTrait {	
	...
	return self::getManyByQuery('WHERE `left` > ? AND `right` < ? AND `children_count` = 0 ORDER BY `left`', [$this->left, $this->right]);
    }
	...
	//SQL
	self::getDb()->query(
		'UPDATE `' . self::getTableName() . '` SET `left` = ?, `right` = ?, `level` = ?, `children_count` = ?  WHERE id = ?;', 
	...
	if( empty($oldRow) or ($oldRow['parent_id'] != $this->parent->id)){
	    $r = self::recTreeUpdate(0, self::getDb()->fetchAll('SELECT `id`, `parent_id`, `left`, `right`, `level` FROM `' . self::getTableName() . '`;'), $left = 1, -1);
	    //var_dump($r);
	    //die('updated?');
	}
    }
