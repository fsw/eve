...
class Field_Image extends Field
{
        ...
        if (!empty($_FILES[$this->name]['tmp_name'])) {   
            ...
            foreach ($output_sizes as $out_size) {
                ...
                switch ($output_format) {
                    case 'gif':
                        imagegif($new_img, $new_path . '.gif');
                        break;
                    case 'jpg':
                        imagejpeg($new_img, $new_path . '.jpg', 90);
                        break;
                    case 'png':
                        imagepng($new_img, $new_path . '.png', 9);
                        break;
                }
            }
        }
    ...
    public function getFormInput ($value) {
        ...
        $ret .= '</div>';
        $ret .= 'alt text: <input type="text" name="' . $this->name . '_alt" value="' . $value->getAlt() . '"/>';
        return $ret;
    }
