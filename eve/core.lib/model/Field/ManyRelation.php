...
class Field_ManyRelation extends Field
{
    ...
    {
        $ret = '<select id="' . $this->name . '" multiple="true" name="' . $this->name .
                 '" class="form-control input-md"' . ($this->isRequired() ? ' required=""' : '') . '>';
    ...
    {
        return []; // isset($_POST[$this->name]) ? new Entity_Promise(($this->to
                       // === 'self') ? $this->entity : $this->to,
                       // $post[$this->name]) : $value;
    }

    public function fromString($string)
    {
        return []; // new Entity_Promise(($this->to === 'self') ? $this->entity
                       // : $this->to, (int)$string);
    }
