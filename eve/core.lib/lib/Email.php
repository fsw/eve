...
class Email
{
    ...
    private static function preparehtmlmail($html, $txt, $boundary, $root)
    {
        ...
        foreach ($images as $path) {
            ...
            // $message_part .= "Content-Disposition: inline;
    ...
    public static function send($to, $subject, $template, $vars = array(), $attach = array())
    {
        ...
        if ($config = Config::get('mail', 'smtp')) {
            ...
            $transport = Swift_SmtpTransport::newInstance($config['host'], $config['port'], $config['encrypt'])->setUsername(
                    $config['username'])
            ...
            $final_msg = self::preparehtmlmail($htmlBody, $txtBody, $boundary, 'mails/' . $template . '/'); // give
                                                                                                            // a
                                                                                                            // function
                                                                                                            // your
                                                                                                            // html*
            
            return mail(is_array($to) ? implode(', ', $to) : $to, $subject, $final_msg['multipart'], 
                    $final_msg['headers']);
        }
    }
}
