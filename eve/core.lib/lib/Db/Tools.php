        ...
        foreach ($tables as $t) {
            ...
            foreach ($create as $row) {
                if (strpos($row, 'KEY') !== false) {
                    ...
                    $current[$t][substr($row, strpos($row, '`') + 1, strrpos($row, '`') - strpos($row, '`') - 1)] = trim(
                            substr($row, strrpos($row, '`') + 1));
                }
            }
        }
        return $current;
    ...
    public function diffStructures($current, $target, $drop = false)
    {
        ...
        foreach ($target as $name => $fields) {
            if (array_key_exists($name, $current)) {
                ...
                $sqls[$name] = 'CREATE TABLE ' . $name . ' (' . PHP_EOL . implode(',' . PHP_EOL, $rows) . PHP_EOL .
